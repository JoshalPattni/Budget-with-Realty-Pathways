...
function calculateTimeToSave() {
    ...
    const buyingPurpose = document.getElementById('buyingPurpose').value;
    let depositPercentage = 0.1; // Default to residential deposit
    if (buyingPurpose === 'no') {
        if (confirm('Are you buying the property to rent out?')) {
            depositPercentage = 0.2; // Buy to let deposit
        }
    }
    const depositAmount = propertyCost * depositPercentage;
    const timeToSave = (depositAmount / monthlySavingsTarget).toFixed(2);
